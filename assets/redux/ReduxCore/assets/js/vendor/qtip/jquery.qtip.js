 ...
 * Plugins: tips modal viemnort svg imagemap ie6
 * Styles: basic css3
 */
            ...
            function( $ ) {
                ...
                CHECKS = PROTOTYPE.checks = {
                    builtin: {
                        ...
                        '^(show|hide|position).(event|target|fixed|inactive|leave|distance|viemnort|adjust)': function() {
                            if ( !this.rendered ) {
                                return;
                            }
                ...
                var rmove = /^position\.(my|at|adjust|target|container|viemnort)|style|content|show\.ready/i,
                ...
                PROTOTYPE.reposition = function( event, effect ) {
                    ...
                    // Target wasn't mouse or absolute...
                        ...
                        // Check if window or document is the target
                            ...
                            if ( target[0] === window ) {
                                position = {
                                    top: (viemnort || target).scrollTop(),
                                    left: (viemnort || target).scrollLeft()
                                };
                            }
                        }
                        ...
                        // Check if the target is an <AREA> element
                            ...
                            pluginCalculations = PLUGINS.imagemap(
                                this, target, at, PLUGINS.viemnort ? method : FALSE
                            );
                        }

                        // Check if the target is an SVG element
                        else if ( PLUGINS.svg && target && target[0].ownerSVGElement ) {
                            pluginCalculations = PLUGINS.svg( this, target, at, PLUGINS.viemnort ? method : FALSE );
                        }
                    ...
                    // Use viemnort adjustment plugin if enabled
                    if ( PLUGINS.viemnort ) {
                        position.adjusted = PLUGINS.viemnort(
                    ...
                    // tooltipmove event
                    if ( !this._trigger( 'move', [position, viemnort.elem || viemnort], event ) ) {
                        return this;
                    }
                ...
                PROTOTYPE.toggle = function( state, event ) {
                    ...
                    if ( state ) {
                        ...
                        // Update the tooltip position (set width first to prevent viemnort/max-width issues)
                        if ( !width ) {
                ...
                PROTOTYPE._assignEvents = function() {
                    var self = this,
                        ...
                        viemnortTarget = posOptions.viemnort,
                    ...
                    if ( posOptions.adjust.resize || viemnortTarget.length ) {
                        this._bind(
                            $.event.special.resize ? viemnortTarget : windowTarget, 'resize', repositionMethod
                        );
                    }

                    // Adjust tooltip position on scroll of the window or viemnort element if present
                ...
                PROTOTYPE._unassignEvents = function() {
                    var targets = [
                        ...
                        this.options.position.viemnort[0],
                ...
                function init( elem, id, opts ) {
                    ...
                    if ( config.position.viemnort === TRUE ) {
                        config.position.viemnort = posOptions.container;
                    }
                ...
                QTIP.defaults = {
                    ...
                    position: {
                        ...
                        effect: function( api, pos, viemnort ) {
                            $( this ).animate(
                                pos, {
                                    duration: 200,
                                    queue: FALSE
                ...
                $.extend(
                    Tip.prototype, {
                        ...
                        reposition: function( event, api, pos, viemnort ) {
                            if ( !this.enabled ) {
                                return;
                            }
                            ...
                            // If our tip position isn't fixed e.g. doesn't adjust with viemnort...
                ...
                ;
                PLUGINS.viemnort = function( api, position, posOptions, targetWidth, targetHeight, elemWidth, elemHeight ) {
                    var target = posOptions.target,
                        ...
                        viemnortWidth, viemnortHeight, viemnortScroll, viemnortOffset;

                    // If viemnort is not a jQuery element, or it's the window/document, or no adjustment method is used... return
                    if ( !viemnort.jquery || target[0] === window || target[0] === document.body || adjust.method === 'none' ) {
                        return adjusted;
                    }
                    ...
                    // Cache our viemnort details
                    fixed = tooltip.css( 'position' ) === 'fixed';
                    viemnortWidth = viemnort[0] === window ? viemnort.width() : viemnort.outerWidth( FALSE );
                    viemnortHeight = viemnort[0] === window ? viemnort.height() : viemnort.outerHeight( FALSE );
                    viemnortScroll = {left: fixed ? 0 : viemnort.scrollLeft(), top: fixed ? 0 : viemnort.scrollTop()};
                    viemnortOffset = viemnort.offset() || adjusted;
                    ...
                    function calculate( side, otherSide, type, adjust, side1, side2, lengthName, targetLength, elemLength ) {
                        var initialPos = position[side1],
                            ...
                            sideOffset = viemnortScroll[side1] + viemnortOffset[side1] - (containerStatic ? 0 : containerOffset[side1]),
                            overflow1 = sideOffset - initialPos,
                            overflow2 = initialPos + elemLength - (lengthName === WIDTH ? viemnortWidth : viemnortHeight) - sideOffset,
                        ...
                        if ( isShift ) {
                            ...
                            // Adjust position but keep it within viemnort dimensions
                            position[side1] += overflow1 > 0 ? overflow1 : overflow2 > 0 ? -overflow2 : 0;
                            position[side1] = Math.max(
                                -containerOffset[side1] + viemnortOffset[side1],
                                initialPos - offset,
                                Math.min(
                                    Math.max(
                                        -containerOffset[side1] + viemnortOffset[side1] + (lengthName === WIDTH ? viemnortWidth : viemnortHeight),
                                        initialPos + offset
                                    ),
                                    position[side1],
                        ...
                        // flip/flipinvert
                            ...
                            if ( position[side1] < viemnortScroll && -position[side1] > overflow2 ) {
                    ...
                    // Adjust position based onviemnort and adjustment options
                    adjusted = {
