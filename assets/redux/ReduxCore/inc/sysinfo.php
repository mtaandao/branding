...
// We need to make sure plugin.php is loaded!
    ...
    if ( ! class_exists( 'Simple_System_Info' ) ) {
        ...
        class Simple_System_Info {
            ...
            /**
             * Get system info
             * Returns the system info for a Mtaandao instance
             *
             * @access      public
             * @author      Daniel J Griffiths
             * @since       1.0.0
             * @global             $mndb
             * @global      object $mndb          Used to query the database
             *
            ...
            public function get( $show_inactive = false, $id = 'system-info-box', $class = null ) {
                ...
                // Get theme info for this Mtaandao version
                ...
                if( !empty( $is_child ) ) {
                    $parent_theme = mn_get_theme( $theme_data->Template );
                ...
                // Make sure mn_remote_post() is working
                ...
                $response = mn_remote_post( 'https://www.paypal.com/cgi-bin/webscr', $params );

                if ( ! is_mn_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
                    $mn_REMOTE_POST = 'mn_remote_post() works';
                } else {
                    $mn_REMOTE_POST = 'mn_remote_post() does not work';
                }

                $return .= 'Remote Post:              ' . $mn_REMOTE_POST . "\n";
                $return .= 'Table Prefix:             ' . 'Length: ' . strlen( $mndb->prefix ) . '   Status: ' . ( strlen( $mndb->prefix ) > 16 ? 'ERROR: Too long' : 'Acceptable' ) . "\n";
                $return .= 'mn_DEBUG:                 ' . ( defined( 'mn_DEBUG' ) ? mn_DEBUG ? 'Enabled' : 'Disabled' : 'Not set' ) . "\n";
                $return .= 'Memory Limit:             ' . mn_MEMORY_LIMIT . "\n";

                if ( has_filter( 'ssi_after_wordpress_config' ) ) {
                    $return .= apply_filters( 'ssi_after_wordpress_config', $return );
                }

                // Mtaandao active plugins
                $return .= "\n" . '-- Mtaandao Active Plugins' . "\n\n";
                ...
                // Mtaandao Multisite active plugins
                if ( is_multisite() ) {
                    $return .= "\n" . '-- Network Active Plugins' . "\n\n";

                    $plugins        = mn_get_active_network_plugins();
                    ...
                    // Mtaandao Multisite inactive plugins
                    if ( $show_inactive == true ) {
