    ...
    if ( ! class_exists( 'Redux_Tracking' ) ) {
        ...
        class Redux_Tracking {
            ...
            public function load( $parent ) {
                ...
                add_action( 'mn_ajax_nopriv_' . $hash, array( $this, 'tracking_arg' ) );
                add_action( 'mn_ajax_' . $hash, array( $this, 'tracking_arg' ) );

                $hash = md5( md5( AUTH_KEY . SECURE_AUTH_KEY . '-redux' ) . '-support' );
                add_action( 'mn_ajax_nopriv_' . $hash, array( $this, 'support_args' ) );
                add_action( 'mn_ajax_' . $hash, array( $this, 'support_args' ) );

                if ( isset( $this->options['allow_tracking'] ) && $this->options['allow_tracking'] == true ) {
                    // The tracking checks daily, but only sends new data every 7 days.
                    if ( ! mn_next_scheduled( 'redux_tracking' ) ) {
                        mn_schedule_event( time(), 'daily', 'redux_tracking' );
                    }
            ...
            function tracking_request() {
                $id    = '#mnadminbar';
                $nonce = mn_create_nonce( 'redux_activate_tracking' );
                                ...
                                setup = function() {
                                    ...
                                    jQuery( '#pointer-close' ).click(
                                        function() {
                                            ...
                                            //redux_setIgnore("tour", "mn-pointer-0", "<?php echo mn_create_nonce('redux-ignore'); ?>");
                                            <?php } else { ?>
            ...
            function trackingObject() {
                ...
                foreach ( get_option( 'active_plugins', array() ) as $plugin_path ) {
                    $plugin_info = get_plugin_data( mn_PLUGIN_DIR . '/' . $plugin_path );
                ...
                $user_query     = new mn_User_Query( array( 'blog_id' => $blog_id, 'count_total' => true, ) );
                $comments_query = new mn_Comment_Query();
                $data           = array(
                    ...
                    'site'      => array(
                        ...
                        'lang'      => get_locale(),
                        'mn_debug'  => ( defined( 'mn_DEBUG' ) ? mn_DEBUG ? true : false : false ),
                        'memory'    => mn_MEMORY_LIMIT,
                    ),
                    'pts'       => $pts,
                    'comments'  => array(
            ...
            function tracking() {
                ...
                if ( ! $data ) {
                    ...
                    $response = mn_remote_post( 'https://redux-tracking.herokuapp.com', $args );
        ...
        function redux_allow_tracking_callback() {
            ...
            if ( mn_verify_nonce( $_REQUEST['nonce'], 'redux_activate_tracking' ) ) {
        ...
        add_action( 'mn_ajax_redux_allow_tracking', 'redux_allow_tracking_callback' );
